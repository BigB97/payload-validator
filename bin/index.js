...
function validator(
  ...
  function validateChildValues(key) {
    ...
    for (var i = 0; i < incomingApiPayload[key].length; i++) {
      if (
        expectedApiPayload[key][0] !== undefined &&
        typeof expectedApiPayload[key][0] !== typeof incomingApiPayload[key][i]
      ) {
        param.response.errorMessage =
          "Parameter '" +
          key +
          "' value expected is '" +
          Object.prototype.toString.call(incomingApiPayload[key]).slice(8, -1) +
          "' of an '" +
          Object.prototype.toString
            .call(expectedApiPayload[key][0])
            .slice(8, -1) +
          "'";
        param.success = false;
      } else {
        arrValue =
          expectedApiPayload[key][0] !== undefined
            ? validator(
                incomingApiPayload[key][i],
                expectedApiPayload[key][0],
                mandatory_elements,
                blank_value
              )
            : incomingApiPayload[key][i];
        ...
        if (!string_value && (arrValue === null || !arrValue.success)) {
          ...
          param.response =
            arrValue === null ||
            arrValue.response === undefined ||
            arrValue.response === null
              ? temp_resp
              : arrValue.response;
        } else {
  ...
  for (var key in expectedApiPayload) {
    if (incomingApiPayload[key] === undefined) {
      ...
      typeof incomingApiPayload[key] !== typeof expectedApiPayload[key] ||
      (typeof expectedApiPayload[key] === 'object' &&
        expectedApiPayload[key] instanceof Array !==
          incomingApiPayload[key] instanceof Array)
    ) {
      invalidValues.push(key);
      param.success = false;
    } else if (
      typeof expectedApiPayload[key] === 'object' &&
      Object.keys(incomingApiPayload[key]).length === 0
    ) {
      ...
      param.elements[key] = {};
      var value = validator(
        incomingApiPayload[key],
        expectedApiPayload[key],
        mandatory_elements,
        blank_value
      );
      if (value.success) {
      ...
      typeof expectedApiPayload[key] === 'string' &&
      incomingApiPayload[key].trim() === ''
    ) {
      ...
      typeof expectedApiPayload[key] === 'number' &&
      incomingApiPayload[key] < 0
    ) {
      param.success = false;
      param.response.errorMessage =
        "Parameter '" + key + "' value must have a positive number";
    } else {
      param.elements[key] = incomingApiPayload[key];
    }
    if (!param.success) {
  ...
  if (undefinedParams.length !== 0) {
    param.response.errorMessage = `${undefinedParams.join(',')} is required.`;
    param.response.errorKey = undefinedParams;
  } else if (nullValues.length !== 0) {
    param.response.errorMessage =
      "Parameter '" + nullValues.join(',') + "' value cannot be null";
    param.response.errorKey = nullValues;
  } else if (invalidValues.length !== 0) {
    param.response.errorMessage = `${invalidValues.join(
      ','
    )} should be an ${Object.prototype.toString
      .call(expectedApiPayload[invalidValues[0]])
      .slice(8, -1)}.`;
    param.response.errorKey = invalidValues;
  } else if (blankParams.length !== 0) {
    param.response.errorMessage =
      "Parameter '" + blankParams.join(',') + "' value passed is blank";
